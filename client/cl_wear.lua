...
local activeFlatTyres   = {}   -- { [partName] = { wheel = X, startKm = Y, burst = bool } }
local flatTyreThreadRunning = false
...
local function resolveWheel(wheel)
    if wheel == nil then
        local tyreIndexes = {0, 1, 4, 5}
        return tyreIndexes[math.random(#tyreIndexes)]
    end
    return wheel
end

local function ensureVehicleControl(vehicle, timeout)
    if not NetworkGetEntityIsNetworked(vehicle) or NetworkHasControlOfEntity(vehicle) then
        return true
    end

    local expiresAt = GetGameTimer() + (timeout or 250)

    repeat
        NetworkRequestControlOfEntity(vehicle)
        Wait(0)
    until NetworkHasControlOfEntity(vehicle) or GetGameTimer() >= expiresAt

    return NetworkHasControlOfEntity(vehicle)
end

local function setVehicleTyreBurstState(vehicle, wheel, onRim, damage)
    if not vehicle or vehicle == 0 or not DoesEntityExist(vehicle) then return false end
    if not ensureVehicleControl(vehicle, 250) then return false end

    if SetVehicleTyresCanBurst then
        SetVehicleTyresCanBurst(vehicle, true)
    end

    if not onRim and IsVehicleTyreBurst(vehicle, wheel, true) then
        SetVehicleTyreFixed(vehicle, wheel)
        Wait(0)
    end

    SetVehicleTyreBurst(vehicle, wheel, onRim, damage or 1000.0)

    return IsVehicleTyreBurst(vehicle, wheel, false)
end

-- Mantem pneus murchos caso o jogo tente converter para aro.
local function startFlatTyrePersistence(vehicle)
    if flatTyreThreadRunning then return end
    flatTyreThreadRunning = true

    CreateThread(function()
        while cache.vehicle == vehicle do
            local hasFlatTyre = false

            for _, state in pairs(activeFlatTyres) do
                if state and not state.burst then
                    hasFlatTyre = true
                    setVehicleTyreBurstState(vehicle, state.wheel, false, state.damage or 250.0)
                end
            end

            if not hasFlatTyre then break end

            Wait(500)
        end

        flatTyreThreadRunning = false
    end)
end
...
-- Estoura pneu
Appliers["tyre_burst"] = function(vehicle, params, partName)
    local state = partName and activeFlatTyres[partName]
    local wheel = state and state.wheel or resolveWheel(params.wheel)

    if not IsVehicleTyreBurst(vehicle, wheel, false) then
        setVehicleTyreBurstState(vehicle, wheel, true, params.damage or 1000.0)
    elseif state and not state.burst then
        setVehicleTyreBurstState(vehicle, wheel, true, params.damage or 1000.0)
    end
...
-- Mantem o pneu murcho sem rasgar/no aro.
Appliers["tyre_flat"] = function(vehicle, params, partName)
    local currentKm  = params._currentKm or 0
    local state      = activeFlatTyres[partName]

    if not state then
        state = {
            wheel   = resolveWheel(params.wheel),
            startKm = currentKm,
            burst   = false,
            damage  = params.damage or params.flatDamage or 250.0,
        }
        activeFlatTyres[partName] = state
    end

    state.burst = false
    state.damage = params.damage or params.flatDamage or state.damage or 250.0
    setVehicleTyreBurstState(vehicle, state.wheel, false, state.damage)
    startFlatTyrePersistence(vehicle)
end
    ...
    for partName, partState in pairs(vehicleParts) do
        ...
        if symptom then
            for _, effect in ipairs(symptom.effects) do
                ...
                if applier then
                    effect.params._currentKm = currentKm
                    applier(vehicle, effect.params, partName)
                    effect.params._currentKm = nil
                end
            end
        else
            stopParticle(partName)
            activeFlatTyres[partName] = nil
        end
